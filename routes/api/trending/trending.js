...
// Validators
const validateTrending = require('../../../validations/trending/trending');

/**
 * @dec       This API makes a request to TMDb API and returns the request
 *            for the client to consume
 * @route     GET /api/trending/all/`week` || `today`/ TODO: Add option to do `week` || `today`
...
router.get('/', (req, res) => {
  ...
  const { errors, isValid } = validateTrending(queryObject);
  if (!isValid) {
    res.status(400);
  ...
  // Get popular movies
  axios
    .get(`/trending/all/week?api_key=${TMDb_API}&language=${language}&page=${page}`)
    .then((response) => {
      const { data } = response;
      res.send(data);
    })
